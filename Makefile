...
$(BUILD_DIR)/$(PROJECT).ngc: $(PROJECT).vhd | $(BUILD_DIR)
	@cd $(BUILD_DIR); \
	echo "run -ifn ../$(PROJECT).vhd -ifmt VHDL -ofn $(PROJECT) -p $(PART) -opt_mode Speed -opt_level 1" | xst

$(BUILD_DIR)/$(PROJECT).ngd: papilio-pro.ucf $(BUILD_DIR)/$(PROJECT).ngc
	@cd $(BUILD_DIR); \
	ngd$(BUILD_DIR) -p $(PART) -uc ../papilio-pro.ucf $(PROJECT).ngc
